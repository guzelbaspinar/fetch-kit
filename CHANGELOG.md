...
- README: clarify that a string `rawBody` may still get a default `Content-Type` from native `fetch`/undici (fetch-kit does not add JSON serialization for `rawBody`).
